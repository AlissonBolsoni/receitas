...
def index(request):
    receitas = Receita.objects.order_by('-data_receita').filter(ativa=True)
    ...
    receitas_filtradas = receitas.filter(nome_receita__icontains=nome_a_buscar)
    paginator = Paginator(receitas_filtradas, 1)
